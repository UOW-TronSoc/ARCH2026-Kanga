...
// Wrist encoder pins (AB quadrature) — assign to free GPIO pairs
#define WRIST_ENC_A 32
#define WRIST_ENC_B 33
...
// Mutex for safe encoder count access across tasks
SemaphoreHandle_t xEncoderMutex = NULL;
...
TaskHandle_t xWristEncoderHandle = NULL;

Servo EEServo;
ESP32Encoder wristEncoder;
...
void setup() {
  ...
  wristEncoder.attachFullQuad(WRIST_ENC_A, WRIST_ENC_B);
  wristEncoder.setCount(0);
...
// 4. J5 Encoder Read — publishes count over CAN at fixed interval
void vWristEncoderRead(void *pvParameters) {
  Serial.println("Enter Wrist Encoder Task");

  CanFrame encoderFrame;
  encoderFrame.identifier = CAN_ID_WRIST_ENCODER;
  encoderFrame.extd = 0;
  encoderFrame.data_length_code = 4;

  for (;;) {
    int32_t count = 0;

    if (xSemaphoreTake(xEncoderMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
      count = (int32_t)wristEncoder.getCount();
      xSemaphoreGive(xEncoderMutex);
    }

    // Pack 32-bit count little-endian into CAN payload
    encoderFrame.data[0] = (count)       & 0xFF;
    encoderFrame.data[1] = (count >> 8)  & 0xFF;
    encoderFrame.data[2] = (count >> 16) & 0xFF;
    encoderFrame.data[3] = (count >> 24) & 0xFF;

    xQueueSend(xTransmitFrame, &encoderFrame, 0);

    Serial.print("Wrist Encoder Count: ");
    Serial.println(count);

    vTaskDelay(pdMS_TO_TICKS(20));  // 50 Hz publish rate
  }
}