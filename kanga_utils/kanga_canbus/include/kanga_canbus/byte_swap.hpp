#ifndef KANGA_CANBUS_BYTE_SWAP_HPP
#define KANGA_CANBUS_BYTE_SWAP_HPP
...
T read_be(const unsigned char* buf) {
    return maybe_byteswap<std::endian::big>(*(T*)buf);
}

template<typename T>
void write_le(const T& val, unsigned char* buf) {
    *(T*)buf = maybe_byteswap<std::endian::little>(val);
}

#endif // KANGA_CANBUS_BYTE_SWAP_HPP
