...
def generate_launch_description() -> LaunchDescription:
    ...
    config_path = os.path.join(package_share, "config", "odrive_node_ids_drive.yaml")
    node_configs = _load_config(config_path)
    default_namespaces = ["wheel_fl", "wheel_bl", "wheel_br", "wheel_fr"]

    launch_actions = []
    for index, node_cfg in enumerate(node_configs):
        namespace = node_cfg.get(
            "namespace",
            default_namespaces[index] if index < len(default_namespaces) else f"wheel_axis{index}",
        )
