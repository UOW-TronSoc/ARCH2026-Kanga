...
void ODriveCanNode::recv_callback(const can_frame& frame) {
    ...
    // RCLCPP_INFO(rclcpp::Node::get_logger(), "Callback triggered");


    if(((frame.can_id >> 5) & 0x3F) != node_id_) return;

    // RCLCPP_INFO(rclcpp::Node::get_logger(), "Got Data");
    switch(frame.can_id & 0x1F) {
        case CmdId::kHeartbeat: {
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Received heartbeat");
            if (!verify_length("kHeartbeat", 8, frame.can_dlc)) break;
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Processing heartbeat");
        ...
        case CmdId::kGetError: {
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Received error");
            if (!verify_length("kGetError", 8, frame.can_dlc)) break;
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Processing error");
        ...
        case CmdId::kGetEncoderEstimates: { 
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Received encoder estimates");
            if (!verify_length("kGetEncoderEstimates", 8, frame.can_dlc)) break;
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Processing encoder estimates");
            std::lock_guard<std::mutex> guard(ctrl_stat_mutex_);
        ...
        case CmdId::kGetIq: {
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Received Iq estimates");
            if (!verify_length("kGetIq", 8, frame.can_dlc)) break;
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Processing Iq estimates");
            std::lock_guard<std::mutex> guard(ctrl_stat_mutex_);
        ...
        case CmdId::kGetTemp: {
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Received temperature estimates");
            if (!verify_length("kGetTemp", 8, frame.can_dlc)) break;
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Processing temperature estimates");
            std::lock_guard<std::mutex> guard(odrv_stat_mutex_);
        ...
        case CmdId::kGetBusVoltageCurrent: {
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Received bus voltage and current estimates");
            if (!verify_length("kGetBusVoltageCurrent", 8, frame.can_dlc)) break;
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Processing bus voltage and current estimates");
            std::lock_guard<std::mutex> guard(odrv_stat_mutex_);
        ...
        case CmdId::kGetTorques: {
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Received torque estimates");
            if (!verify_length("kGetTorques", 8, frame.can_dlc)) break;
            // RCLCPP_INFO(rclcpp::Node::get_logger(), "Processing torque estimates");
            std::lock_guard<std::mutex> guard(ctrl_stat_mutex_);
    ...
    // RCLCPP_INFO(rclcpp::Node::get_logger(), "Processing complete");

    //print ctrl_pub_flag
    // RCLCPP_INFO(rclcpp::Node::get_logger(), "ctrl_pub_flag: %d", ctrl_pub_flag_);

    
    if (ctrl_pub_flag_ == 0b1111 || ctrl_pub_flag_ == 0b0010 || ctrl_pub_flag_ == 3) {
        // RCLCPP_INFO(rclcpp::Node::get_logger(), "Publishing controller status");
        ctrl_publisher_->publish(ctrl_stat_);
        ctrl_pub_flag_ = 0;
    }
    
    if (odrv_pub_flag_ == 0b111) {
        // RCLCPP_INFO(rclcpp::Node::get_logger(), "Publishing ODrive status");
